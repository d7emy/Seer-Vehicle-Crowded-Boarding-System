...
class VisionTracker:
    ...
    def track_and_get_speeds(self, frame, current_time):
        ...
        for box, track_id in zip(boxes, track_ids):
            ...
            self.car_history[track_id].append((real_x, real_y, current_time))
            speed_kmh = 0.0

            if len(self.car_history[track_id]) > self.FPS // 2:
                old_x, old_y, old_time = self.car_history[track_id][0]
            ...
            tracked_objects.append({
                "id": track_id, "bbox": (x1, y1, x2, y2), "cy": cy, "speed": speed_kmh
            })
